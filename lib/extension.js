...
class CppReferenceExtension {
    ...
    async onContentAggregated({playbook, siteAsciiDocConfig, siteCatalog, contentAggregate}) {
        ...
        await this.setupDependencies(playbook)
        await this.setupMrDocs(playbook)
     ...
     */
    async setupDependencies(playbook) {
        const playbookDir = playbook.dir
        const playbookBuildDir = path.join(playbookDir, 'build')
        const generatedFilesDir = path.join(playbookBuildDir, 'cpp-reference-extension')
        const DependenciesDir = path.join(generatedFilesDir, 'dependencies')
        ...
        for (const dependency of dependencies) {
            ...
            const {name, repo, tag, variable} = dependency
            if (!name) {
     ...
     */
    async setupMrDocs(playbook) {
        const playbookDir = playbook.dir
        const playbookBuildDir = path.join(playbookDir, 'build')
        const generatedFilesDir = path.join(playbookBuildDir, 'cpp-reference-extension')
        const mrDocsTreeDir = path.join(generatedFilesDir, 'mrdocs')
    ...
    async processOrigin(origin, componentVersionBucket, playbook, cacheDir, gitCache, managedWorktrees) {
        ...
        for (const collector of collectors) {
            await this.processCollector(collector, origin, componentVersionBucket, playbook, worktreeDir, worktree);
        }
    }
     ...
     * @param worktree
     */
    async processCollector(collector, origin, componentVersionBucket, playbook, worktreeDir, worktree) {
        ...
        const referenceOutputDir =
            version ?
                path.join(generatedFilesDir, 'reference', name, 'versioned', version) :
                path.join(generatedFilesDir, 'reference', name, 'main')
