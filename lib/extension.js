...
function isSkipValueTruthy(raw) {
    if (raw === undefined || raw === null) return false
    if (raw === true) return true
    if (raw === false) return false
    // A bare `--attribute skip-cpp-reference` arrives as `''`; that's the AsciiDoc
    // convention for a bare flag, so treat it as truthy here. Env values filter
    // `''` upstream in isSkipEnvTruthy.
    return !/^(0|false|no|off)$/i.test(String(raw).trim())
}

function isSkipEnvTruthy(raw) {
    if (raw === undefined || raw === null || raw === '') return false
    return isSkipValueTruthy(raw)
}

// Returns every active source, in precedence order (playbook, then cli, then env).
function detectSkipSources(config, playbook) {
    const sources = []
    if (config && config.skip === true) sources.push('playbook')
    const attrs = playbook && playbook.asciidoc && playbook.asciidoc.attributes
    if (attrs && Object.prototype.hasOwnProperty.call(attrs, 'skip-cpp-reference')) {
        if (isSkipValueTruthy(attrs['skip-cpp-reference'])) sources.push('cli')
    }
    if (isSkipEnvTruthy(process.env.ANTORA_SKIP_CPP_REFERENCE)) sources.push('env')
    return sources
}
...
// Set once we patch Antora's page composer, so we don't wrap buildUiModel twice.
let pageComposerBreadcrumbsPatched = false
...
class CppReferenceExtension {
    ...
    constructor(generatorContext, {config, playbook}) {
        ...
        this.autoBaseUrl = config.autoBaseUrl === true
        const breadcrumbsSetting = config.breadcrumbs
        this.breadcrumbsEnabled = breadcrumbsSetting === true
        this.skipReferenceSources = detectSkipSources(config, playbook)
        this.skipReferenceSource = this.skipReferenceSources[0] || null
        this.skipReference = this.skipReferenceSources.length > 0
        this.remoteRefCache = new Map()
        this.githubEventPayload = undefined
        /**
         * Tracks every generated `.adoc` file per component/version/module so we can
         * reconstruct ancestor relationships once Antora classifies the content.
         * @type {Map<string, Set<string>>}
         */
        this.referencePageRegistry = new Map()
        /**
         * Records which component/version/module tuples requested breadcrumb synthesis.
         * @type {Set<string>}
         */
        this.referenceModuleKeys = new Set()
        /**
         * Stores human-friendly labels for each breadcrumb module key.
         * @type {Map<string, {moduleName:string,label:string}>}
         */
        this.referenceModuleMetadata = new Map()
        this.referenceModuleMetadata = new Map()
        CppReferenceExtension.ensurePageComposerBreadcrumbPatch(this.context, this.logger)
        // playbook = playbook
    }

    /**
     * Monkey-patch Antora's page composer so the UI can read our synthetic breadcrumbs.
     *
     * @param {Object} generatorContext - Current Antora generator context (gives us require()).
     * @param {Object} logger - Logger used for warnings.
     */
    static ensurePageComposerBreadcrumbPatch(generatorContext, logger) {
        if (pageComposerBreadcrumbsPatched) {
            return
        }
        if (!generatorContext || typeof generatorContext.require !== 'function') {
            logger?.warn?.('Cannot patch page composer breadcrumbs: generator context is unavailable')
            return
        }
        try {
            const modulePath = '@antora/page-composer/lib/build-ui-model'
            const buildUiModelModule = generatorContext.require(modulePath)
            const originalBuildUiModel = buildUiModelModule?.buildUiModel
            if (typeof originalBuildUiModel !== 'function') {
                logger?.warn?.('Cannot patch page composer breadcrumbs: buildUiModel is unavailable')
                return
            }
            if (originalBuildUiModel.__cpprefBreadcrumbPatched) {
                pageComposerBreadcrumbsPatched = true
                return
            }
            const patched = function patchedBuildUiModel(baseUiModel, file, contentCatalog, navigationCatalog) {
                const uiModel = originalBuildUiModel(baseUiModel, file, contentCatalog, navigationCatalog)
                if (uiModel?.page && file?.cpprefBreadcrumbs?.length) {
                    const existingLength = Array.isArray(uiModel.page.breadcrumbs) ? uiModel.page.breadcrumbs.length : 0
                    if (!existingLength || file.cpprefBreadcrumbs.length > existingLength) {
                        uiModel.page.breadcrumbs = file.cpprefBreadcrumbs
                    }
                }
                return uiModel
            }
            patched.__cpprefBreadcrumbPatched = true
            buildUiModelModule.buildUiModel = patched
            pageComposerBreadcrumbsPatched = true
            logger?.debug?.('Patched Antora page composer to honor synthetic breadcrumbs')
        } catch (err) {
            logger?.warn?.(
                `Failed to enable synthetic breadcrumbs integration: ${err && err.message ? err.message : err}`
            )
        }
    }
    ...
    async onContentAggregated({playbook, siteAsciiDocConfig, siteCatalog, contentAggregate}) {
        if (this.skipReference) {
            await this.handleSkipReference(playbook, contentAggregate)
            return
        }
        const configBreadcrumbs = this.config?.breadcrumbs
        if (!this.breadcrumbsEnabled && configBreadcrumbs === true) {
    ...
    /**
     * Short-circuit the reference pipeline when skip mode is on. For each
     * bucket we drop anything previously generated under the target module
     * and write a placeholder `index.adoc` so cross-module xrefs (typically
     * nav) still resolve. We also register and finalize on the tagfile
     * registry without publishing entries, so a co-installed tagfiles
     * consumer's `waitFor('reference')` doesn't deadlock.
     */
    async handleSkipReference(playbook, contentAggregate) {
        this.tagfileRegistry = CppReferenceExtension.ensureTagfileRegistry(playbook, this.logger)
        this.tagfileRegistry?.registerProducer?.(TAGFILE_REGISTRY_PRODUCER_ID)
        try {
            const buckets = contentAggregate || []
            let injected = 0
            for (const bucket of buckets) {
                if (!bucket || !Array.isArray(bucket.files)) continue
                const descriptor = bucket.origins?.find?.((o) => o?.descriptor)?.descriptor
                const moduleName = CppReferenceExtension.resolveDefaultModuleName(this.config, descriptor)
                const modulePrefix = `modules/${moduleName}/`
                bucket.files = bucket.files.filter((file) => {
                    const filePath = (file && (file.path || (file.src && file.src.path))) || ''
                    return !filePath.startsWith(modulePrefix)
                })
                bucket.files.push(
                    CppReferenceExtension.buildSkipPlaceholderFile(bucket, this.skipReferenceSources, moduleName)
                )
                injected += 1
            }
            this.logger.info(
                `Reference generation skipped (sources=${this.skipReferenceSources.join(',')}); ` +
                `inserted ${injected} placeholder index page(s).`
            )
        } finally {
            this.tagfileRegistry?.finalize?.(TAGFILE_REGISTRY_PRODUCER_ID)
        }
    ...
    /**
     * Resolve the module name where reference pages should land. Per-component
     * descriptor overrides win over the playbook-level extension config.
     */
    static resolveDefaultModuleName(config, descriptor) {
        const descRef = descriptor?.ext?.cppReference
        const fromDescriptor = descRef && !Array.isArray(descRef) && typeof descRef === 'object'
            ? (descRef.module || descRef.moduleName)
            : undefined
        return (
            fromDescriptor ||
            config?.module ||
    ...
    /**
     * Build the placeholder index page used by skip mode. Its only job is
     * to keep `xref:<module>:index.adoc[]` macros from other modules
     * (typically the project-wide nav) resolving.
     */
    static buildSkipPlaceholderFile(bucket, sources, moduleName) {
        const origin = (bucket && Array.isArray(bucket.origins) && bucket.origins[0]) || {}
        const resolvedModule = moduleName || REFERENCE_MODULE_NAME
        const relpath = `modules/${resolvedModule}/pages/index.adoc`
        const contents = Buffer.from(CppReferenceExtension.buildSkipPlaceholderBody(sources), 'utf8')
        const basename = 'index.adoc'
        const stem = 'index'
        const extname = '.adoc'
        return {
            path: relpath,
            ...
            // `scanned` and `realpath` mirror scanDirectories' no-worktree branch.
            src: {
                path: relpath,
                basename,
                stem,
    ...
    /**
     * Build the AsciiDoc body of the skip-mode placeholder. Accepts a single
     * source string or an array; when multiple sources are active the body
     * lists an undo step for each one.
     */
    static buildSkipPlaceholderBody(sources) {
        const undoFor = {
            playbook: 'Set `skip: false` on this extension in the playbook',
            cli: 'Drop `--attribute skip-cpp-reference` from the antora CLI',
            env: 'Unset the `ANTORA_SKIP_CPP_REFERENCE` environment variable',
        }
        const list = Array.isArray(sources) ? sources.filter(Boolean) : [sources].filter(Boolean)
        const lines = [
            '= Reference',
            '',
            'Reference generation was skipped for this preview build.',
            '',
        ]
        if (list.length <= 1) {
            const undo = undoFor[list[0]] || 'Disable the skip flag'
            lines.push(`${undo} and rebuild to produce the full reference.`)
        } else {
            lines.push('Multiple sources enabled skip mode. Undo each before rebuilding to produce the full reference:')
            lines.push('')
            for (const src of list) {
                const undo = undoFor[src] || `Disable the ${src} skip flag`
                lines.push(`* ${undo}.`)
            }
        }
        lines.push('')
    ...
    /**
     * Inject breadcrumb metadata once Antora finishes classifying the content catalog.
     *
     * @param {Object} params
     * @param {Object} params.contentCatalog
     */
    async onContentClassified({contentCatalog}) {
        if (!this.breadcrumbsEnabled) {
            return
        }
        if (!this.referenceModuleKeys.size) {
            this.logger.debug('Synthetic breadcrumbs skipped: no eligible reference modules')
            return
        }
        if (!contentCatalog || typeof contentCatalog.getPages !== 'function') {
            this.logger.debug('Skipping synthetic breadcrumbs: content catalog unavailable')
            return
        }
        try {
            this.applySyntheticBreadcrumbs(contentCatalog)
        } catch (err) {
            this.logger.warn(`Failed to apply synthetic breadcrumbs: ${err && err.message ? err.message : err}`)
        }
    }

    /**
     * Iterate the content catalog and assign breadcrumbs to every recorded reference page.
     *
     * @param {Object} contentCatalog
     */
    applySyntheticBreadcrumbs(contentCatalog) {
        ...
        for (const page of pages) {
            ...
            const normalizedRelative = CppReferenceExtension.normalizeReferenceRelativePath(src.relative, src.module)
            if (!normalizedRelative) {
                continue
            }
            const key = CppReferenceExtension.composeReferencePageKey(
                src.component,
                CppReferenceExtension.normalizeVersion(src.version),
                src.module
            )
            const recordedSet = this.referencePageRegistry.get(key)
            if (!recordedSet || !recordedSet.has(normalizedRelative)) {
                continue
            }
            let moduleState = modulePages.get(key)
            if (!moduleState) {
                moduleState = {
                    paths: recordedSet,
                    pages: new Map(),
                    meta: this.referenceModuleMetadata.get(key)
                }
                modulePages.set(key, moduleState)
            }
            moduleState.pages.set(normalizedRelative, page)
        }
        for (const moduleState of modulePages.values()) {
            CppReferenceExtension.assignSyntheticBreadcrumbs(
                moduleState.paths,
                moduleState.pages,
                moduleState.meta,
                this.logger
    ...
    static parseMrDocsVersionFromOutput(versionBuf) {
        ...
        // Try to extract the version token from common formats:
        // - "version: 0.0.4+ad1e7b..."
        // - "MrDocs version 0.8.0+45571ab5219b.modified"
        // - standalone semver-ish fragment in the output
        let raw = ''
        for (const line of lines) {
            const explicit = line.match(/\bversion\b[:\s]*([^\s].*)$/i)
            if (explicit) {
                raw = explicit[1].trim()
                break
            }
            const semverish = line.match(/[vV]?\d+\.\d+\.\d+[0-9A-Za-z.+-]*/)
            if (semverish) {
                raw = semverish[0].trim()
                break
            }
        }

        // If nothing matched above, fall back to the whole output.
        raw = raw || text.trim()
        ...
        // Normalize common prefixes (MrDocs version X, leading "version", or "v")
        const normalized =
            raw
                .replace(/^mrdocs\s+/i, '')
                .replace(/^version\s*/i, '')
        ...
        // Try a few candidates for coercion to tolerate stray suffixes like "+dirty" or ".modified"
        const candidates = [normalized, raw, text]
        for (const candidate of candidates) {
            const coerced = semver.coerce(candidate, { loose: true })
    ...
    async processOrigin(origin, componentVersionBucket, playbook, cacheDir, gitCache, managedWorktrees) {
        ...
        const defaultModuleName = CppReferenceExtension.resolveDefaultModuleName(this.config, descriptor)
        ...
        for (const collectorConfig of collectorConfigs) {
            ...
            const collectorAutoBase =
                collectorConfig && Object.prototype.hasOwnProperty.call(collectorConfig, 'autoBaseUrl') ?
                    collectorConfig.autoBaseUrl === true :
                    undefined

            collectors.push({
                config: mrdocsConfigFile,
                module: collectorModule,
                autoBaseUrl: collectorAutoBase
            })
        }
    ...
    async prepareWorktree(repo) {
        ...
        try {
            ...
            try {
                ...
                await CppReferenceExtension.removeSymlinks(worktreeDir)
                this.logger.debug(`Removed symlinks from ${worktreeDir}`)
            } catch {
    ...
    /**
     * Removes every symbolic link under a worktree, leaving the .git
     * directory alone.
     *
     * The same worktree directory is reused for every ref of a repository.
     * isomorphic-git creates symlinks with symlink(2), which fails with
     * EEXIST when a link already exists at that path, even when the forced
     * checkout is trying to point it somewhere else (a ref that moved the
     * file a link targets). Removing the links first lets the checkout
     * recreate them for the new ref, unchanged ones included, since a
     * forced checkout rewrites every entry missing from the worktree.
     *
     * @param {string} dir - The directory to clean recursively.
     * @returns {Promise<void>}
     */
    static async removeSymlinks(dir) {
        const entries = await fsp.readdir(dir, {withFileTypes: true})
        await Promise.all(
            entries.map(async (entry) => {
                if (entry.name === '.git') return
                const entryPath = ospath.join(dir, entry.name)
                if (entry.isSymbolicLink()) {
                    await fsp.unlink(entryPath)
                } else if (entry.isDirectory()) {
                    await CppReferenceExtension.removeSymlinks(entryPath)
                }
            })
        )
    }
    ...
    async processCollector(collector, origin, componentVersionBucket, playbook, worktreeDir, worktree, cacheDir) {
        ...
        const moduleName = collector.module || CppReferenceExtension.resolveDefaultModuleName(this.config)
        const autoBaseUrlEnabled =
            typeof collector.autoBaseUrl === 'boolean' ? collector.autoBaseUrl : this.autoBaseUrl
        this.registerBreadcrumbModule(componentVersionBucket, moduleName)
        ...
        const mrDocsArgs = await this.generateMrDocsArgs(collector.config, referenceOutputDir, {
            autoBaseUrlEnabled,
            origin,
            worktreeDir
        })

        await this.runCommand(this.MrDocsExecutable, mrDocsArgs, {cwd: worktreeDir, quiet: playbook.runtime?.quiet})
    ...
    /**
     * Build the MrDocs CLI argv list for a given collector run.
     *
     * @param {string} collectorConfig - Absolute path to the mrdocs.yml file.
     * @param {string} referenceOutputDir - Directory where MrDocs should write artifacts.
     * @param {Object} [options]
     * @param {boolean} [options.autoBaseUrlEnabled=false] - Whether to attempt GitHub base URL detection.
     * @param {Object} [options.origin] - Origin metadata used for auto base URL heuristics.
     * @param {string} [options.worktreeDir] - Worktree directory used as git cwd for heuristics.
     * @returns {Promise<string[]>} Array of command-line arguments passed to MrDocs.
     */
    async generateMrDocsArgs(collectorConfig, referenceOutputDir, options = {}) {
        const {autoBaseUrlEnabled, origin, worktreeDir} = options
        const args = []
        ...
        // Generator
        const mrDocsVersion = typeof this.MrDocsVersion === 'string' ? this.MrDocsVersion : ''
        const versionIsMaster = mrDocsVersion.includes('master')
        const versionIsDevelop = mrDocsVersion.includes('develop')
        const versionIsSemver = mrDocsVersion && semver.valid(mrDocsVersion, {loose: true, includePrerelease: true})
        if (versionIsMaster || versionIsDevelop) {
            args.push(`--generator=adoc`)
        } else if (versionIsSemver) {
            if (semver.satisfies(mrDocsVersion, '>=0.0.3', {loose: true, includePrerelease: true}))
            {
                args.push(`--generator=adoc`)
            }
            else {
        ...
        if (autoBaseUrlEnabled) {
            const resolvedBaseUrl = await this.resolveAutoBaseUrl({origin, worktreeDir})
            if (resolvedBaseUrl) {
                args.push(`--base-url=${resolvedBaseUrl}`)
            } else {
                this.logger.debug('autoBaseUrl enabled but no GitHub candidate passed validation; leaving base-url untouched')
            }
        }
    ...
    /**
     * Derive a stable GitHub blob URL for the current worktree.
     *
     * The resolver inspects GitHub Actions metadata, optional CPPREF_* overrides,
     * and local git state, verifying each candidate via `git ls-remote` before
     * returning a usable base URL. Non-GitHub hosts are ignored.
     *
     * @param {Object} options
     * @param {Object} options.origin - Antora origin describing the component repo.
     * @param {string} options.worktreeDir - Absolute path to the prepared worktree.
     * @returns {Promise<string|null>} Resolved base URL or null when no candidate verifies.
     */
    async resolveAutoBaseUrl({origin, worktreeDir}) {
        const env = process.env || {}
        const serverOrigin = CppReferenceExtension.resolveGitHubServerOrigin(env, origin)
        if (!serverOrigin) {
            this.logger.debug('autoBaseUrl currently only supports github.com hosts; skipping override')
            return null
        }

        const repoCandidates = []
        const seenRepos = new Set()
        const addRepoCandidate = (value, source) => {
            if (!value) {
                return
            }
            let normalized = CppReferenceExtension.normalizeGitHubRepoSlug(value)
            if (!normalized) {
                const parsed = CppReferenceExtension.extractGitHubRepoFromUrl(value)
                normalized = parsed?.repo
            }
            if (!normalized || seenRepos.has(normalized)) {
                return
            }
            repoCandidates.push({slug: normalized, source})
            seenRepos.add(normalized)
        }

        const refCandidates = []
        const seenRefs = new Set()
        const addRefCandidate = (value, type, source, repo) => {
            if (!value || (type !== 'commit' && type !== 'branch')) {
                return
            }
            ...
            const normalizedRepo = repo ? CppReferenceExtension.normalizeGitHubRepoSlug(repo) : undefined
            const key = `${type}:${normalizedRepo || '*'}:${trimmed}`
            if (seenRefs.has(key)) {
                return
            }
            refCandidates.push({
                ref: type === 'commit' ? trimmed.toLowerCase() : trimmed,
                type,
                source,
                repo: normalizedRepo
            })
        ...
        const eventPayload = this.loadGitHubEventPayload()
        const pullRequest = eventPayload?.pull_request
        if (pullRequest) {
            const headRepo = pullRequest.head?.repo?.full_name
            addRepoCandidate(headRepo, 'github pull_request head repo')
            addRefCandidate(pullRequest.head?.sha, 'commit', 'github pull_request head sha', headRepo)
            addRefCandidate(pullRequest.head?.ref, 'branch', 'github pull_request head ref', headRepo)
        }

        addRepoCandidate(env.CPPREF_GITHUB_REPOSITORY, 'CPPREF_GITHUB_REPOSITORY')
        addRepoCandidate(env.GITHUB_REPOSITORY, 'GITHUB_REPOSITORY')

        const originRepo = CppReferenceExtension.extractGitHubRepoFromUrl(origin?.url)
        if (originRepo?.repo) {
            addRepoCandidate(originRepo.repo, 'origin.url')
        }

        if (worktreeDir) {
            const remoteUrl = await this.readGitOutput(['remote', 'get-url', 'origin'], worktreeDir)
            const remoteRepo = CppReferenceExtension.extractGitHubRepoFromUrl(remoteUrl)
            if (remoteRepo?.repo) {
                addRepoCandidate(remoteRepo.repo, 'git remote origin')
            }
        }

        addRefCandidate(env.CPPREF_GITHUB_SHA, 'commit', 'CPPREF_GITHUB_SHA')
        addRefCandidate(env.GITHUB_SHA, 'commit', 'GITHUB_SHA')
        addRefCandidate(env.GITHUB_HEAD_REF, 'branch', 'GITHUB_HEAD_REF', pullRequest?.head?.repo?.full_name)
        addRefCandidate(env.CPPREF_GITHUB_REF, 'branch', 'CPPREF_GITHUB_REF')
        if (env.GITHUB_REF_NAME) {
            addRefCandidate(env.GITHUB_REF_NAME, 'branch', 'GITHUB_REF_NAME')
        }
        if (env.GITHUB_REF) {
            addRefCandidate(CppReferenceExtension.extractGitRefName(env.GITHUB_REF), 'branch', 'GITHUB_REF')
        }

        if (worktreeDir) {
            addRefCandidate(await this.readGitOutput(['rev-parse', 'HEAD'], worktreeDir), 'commit', 'git rev-parse HEAD')
            addRefCandidate(await this.readGitOutput(['branch', '--show-current'], worktreeDir), 'branch', 'git branch --show-current')
        }

        if (!repoCandidates.length || !refCandidates.length) {
            return null
        }

        for (const refCandidate of refCandidates) {
            const reposToCheck = refCandidate.repo ?
                repoCandidates.filter((candidate) => candidate.slug === refCandidate.repo) :
                repoCandidates
            for (const repoCandidate of reposToCheck) {
                const verified = await this.verifyGitHubCandidate(serverOrigin, repoCandidate.slug, refCandidate)
                if (verified) {
                    const baseUrl = CppReferenceExtension.buildGitHubBlobBase(serverOrigin, repoCandidate.slug, refCandidate.ref)
                    if (baseUrl) {
                        this.logger.debug(
                            `autoBaseUrl resolved ${baseUrl} from ${repoCandidate.source || 'repository'} + ${refCandidate.source || 'ref'}`
                        )
                        return baseUrl
                    }
                }
    ...
    /**
     * Executes a git command and returns trimmed stdout, swallowing failures.
     *
     * @param {string[]} args - Git arguments, e.g., ['rev-parse', 'HEAD'].
     * @param {string} cwd - Repository directory used as git working directory.
     * @returns {Promise<string|null>} Trimmed stdout on success, otherwise null.
     */
    async readGitOutput(args, cwd) {
        if (!cwd) {
            return null
        }
        try {
            const output = await this.runCommand('git', args, {cwd})
            const text = output?.toString('utf8').trim()
            return text || null
        } catch (err) {
            this.logger.debug(`git ${args.join(' ')} failed: ${err.message}`)
            return null
        }
    }

    /**
     * Runs `git ls-remote` once per remote and caches the parsed refs.
     *
     * @param {string} remoteUrl - Remote URL (https or ssh) to inspect.
     * @returns {Promise<{hashes:Set<string>, refs:Set<string>}|null>} Parsed refs or null if git fails.
     */
    async fetchGitRemoteRefs(remoteUrl) {
        if (!remoteUrl) {
            return null
        }
        if (this.remoteRefCache.has(remoteUrl)) {
            return this.remoteRefCache.get(remoteUrl)
        }
        let parsed = null
        try {
            const output = await this.runCommand('git', ['ls-remote', remoteUrl])
            parsed = CppReferenceExtension.parseLsRemoteOutput(output)
        } catch (err) {
            this.logger.debug(`git ls-remote ${remoteUrl} failed: ${err.message}`)
        }
        this.remoteRefCache.set(remoteUrl, parsed)
        return parsed
    }

    /**
     * Confirms that a commit or branch exists on the remote GitHub repository.
     *
     * @param {string} serverOrigin - GitHub origin (e.g., https://github.com).
     * @param {string} repoSlug - owner/repo slug.
     * @param {Object} candidate - Candidate reference descriptor.
     * @param {string} candidate.ref - Commit hash or branch name.
     * @param {string} candidate.type - 'commit' or 'branch'.
     * @returns {Promise<boolean>} True when the reference exists upstream.
     */
    async verifyGitHubCandidate(serverOrigin, repoSlug, candidate) {
        const normalizedRepo = CppReferenceExtension.normalizeGitHubRepoSlug(repoSlug)
        if (!normalizedRepo) {
            return false
        }
        if (!serverOrigin) {
        ...
        const remoteUrl = `${serverOrigin.replace(/\/$/, '')}/${normalizedRepo}.git`
        const remoteRefs = await this.fetchGitRemoteRefs(remoteUrl)
        if (!remoteRefs) {
            return false
        }
        if (candidate.type === 'commit') {
            return remoteRefs.hashes.has(candidate.ref.toLowerCase())
        }
        const wantedRef = candidate.ref.startsWith('refs/') ? candidate.ref : `refs/heads/${candidate.ref}`
        return remoteRefs.refs.has(wantedRef)
    }

    /**
     * Lazily reads and parses the GitHub Actions event payload when available.
     *
     * @returns {Object|null} Parsed payload or null if absent/unreadable.
     */
    loadGitHubEventPayload() {
        if (this.githubEventPayload !== undefined) {
            return this.githubEventPayload
        }
        const eventPath = process.env.GITHUB_EVENT_PATH
        if (!eventPath) {
            this.githubEventPayload = null
            return null
        }
        try {
            const contents = fs.readFileSync(eventPath, 'utf8')
            this.githubEventPayload = JSON.parse(contents)
        } catch (err) {
            this.logger.debug(`Unable to parse GitHub event payload at ${eventPath}: ${err.message}`)
            this.githubEventPayload = null
        }
        return this.githubEventPayload
    }
    ...
    /**
     * Converts `git ls-remote` stdout into quick-lookup sets.
     *
     * @param {Buffer|string} buffer - Raw command output.
     * @returns {{hashes:Set<string>, refs:Set<string>}} Parsed hashes and refs.
     */
    static parseLsRemoteOutput(buffer) {
        ...
        for (const line of text.split(/\r?\n/)) {
            ...
            if (parts[0]) {
                hashes.add(parts[0].toLowerCase())
            }
            if (parts[1]) {
                refs.add(parts[1])
            }
    ...
    /**
     * Determines which GitHub host to target based on env overrides or origin URL.
     *
     * @param {Object} env - Process environment snapshot.
     * @param {Object} origin - Antora origin containing repo metadata.
     * @returns {string|null} Canonical server origin or null for unsupported hosts.
     */
    static resolveGitHubServerOrigin(env, origin) {
        const preferredServer = env?.CPPREF_GITHUB_SERVER_URL || env?.GITHUB_SERVER_URL
        if (preferredServer) {
            try {
                const parsed = new URL(preferredServer)
                if (CppReferenceExtension.isGitHubHost(parsed.hostname)) {
        ...
        const originRepo = CppReferenceExtension.extractGitHubRepoFromUrl(origin?.url)
        if (originRepo?.server) {
            try {
                const parsed = new URL(originRepo.server)
                if (CppReferenceExtension.isGitHubHost(parsed.hostname)) {
                    return parsed.origin
                }
                return null
    ...
    /**
     * Returns true when the hostname maps to github.com.
     *
     * @param {string} hostname
     * @returns {boolean}
     */
    static isGitHubHost(hostname) {
        if (!hostname) {
            return false
        }
    ...
    /**
     * Normalizes various repo strings (URLs, slugs) into owner/repo form.
     *
     * @param {string} candidate
     * @returns {string|null}
     */
    static normalizeGitHubRepoSlug(candidate) {
        if (!candidate || typeof candidate !== 'string') {
            return null
        }
        let slug = candidate.trim()
        if (!slug) {
            return null
        }
        if (slug.startsWith('/')) {
    ...
    /**
     * Extracts the owner/repo slug and server origin from a GitHub URL or SSH remote.
     *
     * @param {string} candidate
     * @returns {{repo:string, server:string}|null}
     */
    static extractGitHubRepoFromUrl(candidate) {
        if (!candidate || typeof candidate !== 'string') {
            return null
        }
        if (candidate.startsWith('git@')) {
            const match = candidate.match(/^git@([^:]+):(.+?)(?:\.git)?$/)
            if (!match) {
                return null
            }
            if (!CppReferenceExtension.isGitHubHost(match[1])) {
                return null
            }
            const repo = CppReferenceExtension.normalizeGitHubRepoSlug(match[2])
            if (!repo) {
                return null
            }
            return {repo, server: 'https://github.com'}
        }
        try {
            ...
            const repo = CppReferenceExtension.normalizeGitHubRepoSlug(parsed.pathname.replace(/^\/+/, ''))
            if (!repo) {
                return null
            }
            return {repo, server: `${parsed.protocol}//${parsed.host}`}
        } catch {
            return null
        }
    }
    ...
    /**
     * Removes leading ref prefixes (refs/heads, refs/tags, etc.).
     *
     * @param {string} ref
     * @returns {string|null}
     */
    static extractGitRefName(ref) {
    ...
    /**
     * Formats the `https://host/owner/repo/blob/<ref>/` base used by MrDocs.
     *
     ...
     * @returns {string|null}
     */
    static buildGitHubBlobBase(serverOrigin, repoSlug, ref) {
        if (!serverOrigin || !repoSlug || !ref) {
            return null
        }
        const normalizedRepo = CppReferenceExtension.normalizeGitHubRepoSlug(repoSlug)
        if (!normalizedRepo) {
        ...
        const trimmedOrigin = serverOrigin.replace(/\/$/, '')
        return `${trimmedOrigin}/${normalizedRepo}/blob/${encodedRef}/`
    }
    ...
    async scanDirectories(referenceOutputDir, moduleName, origin, componentVersionBucket, worktreeDir, worktree, files) {
        ...
        for (const file of filesToProcess) {
            ...
            if (this.breadcrumbsEnabled && relpath && relpath.endsWith('.adoc')) {
                this.recordReferencePage(componentVersionBucket, resolvedModuleName, relpath)
            }
    ...
    /**
     * Register a component/module combination that should receive synthetic breadcrumbs.
     *
     * @param {Object} componentVersionBucket - Antora component bucket descriptor.
     * @param {string} moduleName - Module name resolving to modules/<module>/pages.
     */
    registerBreadcrumbModule(componentVersionBucket, moduleName) {
        const component = componentVersionBucket?.name || ''
        const version = CppReferenceExtension.normalizeVersion(componentVersionBucket?.version)
        const moduleKey = moduleName || REFERENCE_MODULE_NAME
        const registryKey = CppReferenceExtension.composeReferencePageKey(component, version, moduleKey)
        this.referenceModuleKeys.add(registryKey)
        if (!this.referenceModuleMetadata.has(registryKey)) {
            this.referenceModuleMetadata.set(registryKey, {
                moduleName: moduleKey,
                label: CppReferenceExtension.formatModuleLabel(moduleKey)
            })
        }
    }

    /**
     * Record a generated reference page so the breadcrumb resolver can find it later.
     *
     * @param {Object} componentVersionBucket - Bucket describing the component/version.
     * @param {string} moduleName - Module that received the generated page.
     * @param {string} relativePath - MrDocs relative path within modules/<module>/pages.
     */
    recordReferencePage(componentVersionBucket, moduleName, relativePath) {
        if (!componentVersionBucket || !relativePath) {
            return
        }
        const component = componentVersionBucket.name || ''
        const version = CppReferenceExtension.normalizeVersion(componentVersionBucket.version)
        const moduleKey = moduleName || REFERENCE_MODULE_NAME
        const registryKey = CppReferenceExtension.composeReferencePageKey(component, version, moduleKey)
        let recorded = this.referencePageRegistry.get(registryKey)
        if (!recorded) {
            recorded = new Set()
            this.referencePageRegistry.set(registryKey, recorded)
        }
        recorded.add(relativePath)
    }

    static composeReferencePageKey(component, version, moduleName) {
        return `${component || ''}::${version || ''}::${moduleName || ''}`
    }

    /**
     * Generate a display label for a module name (e.g., api-ref -> Api ref).
     *
     * @param {string} moduleName
     * @returns {string}
     */
    static formatModuleLabel(moduleName) {
        if (!moduleName) return 'Reference'
        const sanitized = moduleName.replace(/[-_]/g, ' ')
    ...
    /**
     * Normalize Antora's version field so falsy/undefined values become '' (unversioned).
     *
    ...
    /**
     * Strip module prefix from a file path so we can compare it to MrDocs output.
     *
     * @param {string} relative - Page src.relative value.
     * @param {string} moduleName - Module the page belongs to.
     * @returns {string|null}
     */
    static normalizeReferenceRelativePath(relative, moduleName) {
        if (!relative) {
            return null
        }
        let normalized = posixify ? relative.replace(/\\/g, '/') : relative
        const prefix = path.join('modules', moduleName || REFERENCE_MODULE_NAME, 'pages')
    ...
    /**
     * Populate breadcrumb arrays for every recorded page inside a module bucket.
     *
     * @param {Set<string>} recordedPaths - Known `.adoc` paths emitted by MrDocs.
     * @param {Map<string,Object>} pageMap - Antora virtual pages keyed by normalized path.
     * @param {Object} moduleMeta - Metadata describing the module label/name.
     * @param {Object} logger - Logger for debug output.
     */
    static assignSyntheticBreadcrumbs(recordedPaths, pageMap, moduleMeta, logger) {
        if (!recordedPaths || !recordedPaths.size || !pageMap || !pageMap.size) {
            return
        }
        for (const [relative, page] of pageMap.entries()) {
            const ancestors = CppReferenceExtension.resolveReferenceAncestors(relative, recordedPaths)
            const crumbs = []
            for (const ancestor of ancestors) {
                const ancestorPage = pageMap.get(ancestor)
                if (!ancestorPage) {
                    logger?.debug?.(`Synthetic breadcrumbs: missing ancestor page for ${ancestor}`)
                    continue
                }
                const ancestorCrumb = ancestor === 'index.adoc'
                    ? CppReferenceExtension.buildModuleBreadcrumbEntry(
                        ancestorPage,
                        moduleMeta
                    )
                    : CppReferenceExtension.buildPageBreadcrumbEntry(ancestorPage)
                if (ancestorCrumb) {
                    crumbs.push(ancestorCrumb)
                }
            }
            const selfCrumb = CppReferenceExtension.buildPageBreadcrumbEntry(page)
            if (selfCrumb) {
                crumbs.push(selfCrumb)
            }
            if (crumbs.length) {
                const deduped = CppReferenceExtension.deduplicateBreadcrumbs(crumbs)
                page.breadcrumbs = deduped
                page.cpprefBreadcrumbs = deduped
            }
        }
    }

    /**
     * Return an ordered list of ancestor paths (index.adoc or foo.adoc) for a page.
     *
     * @param {string} relativePath
     * @param {Set<string>} recordedPaths
     * @returns {string[]}
     */
    static resolveReferenceAncestors(relativePath, recordedPaths) {
        if (!relativePath || !recordedPaths || !recordedPaths.size) {
            return []
        }
        const ancestors = []
        const visitedDirs = new Set()
        let currentDir = path.dirname(relativePath)
        while (currentDir && currentDir !== '.' && !visitedDirs.has(currentDir)) {
            visitedDirs.add(currentDir)
            const indexCandidate = path.join(currentDir, 'index.adoc')
            if (indexCandidate !== relativePath && recordedPaths.has(indexCandidate)) {
                ancestors.push(indexCandidate)
            }
            const fileCandidate = `${currentDir}.adoc`
            if (fileCandidate !== relativePath && recordedPaths.has(fileCandidate)) {
                ancestors.push(fileCandidate)
            }
        ...
        if (relativePath !== 'index.adoc' && recordedPaths.has('index.adoc')) {
            ancestors.push('index.adoc')
        }
        return ancestors.reverse()
    }

    /**
     * Build the breadcrumb entry that represents the module root (Reference, API, etc.).
     *
     * @param {Object} page - Antora virtual page representing modules/<module>/pages/index.adoc.
     * @param {Object} moduleMeta - Metadata captured when the module was registered.
     * @returns {{content:string,url:?string,urlType:?string}|null}
     */
    static buildModuleBreadcrumbEntry(page, moduleMeta) {
        const content = moduleMeta?.label || CppReferenceExtension.formatModuleLabel(moduleMeta?.moduleName)
        const url = CppReferenceExtension.computePageUrl(page)
        return content ? {content, url, urlType: url ? 'internal' : undefined} : null
    }

    /**
     * Build a breadcrumb entry for any concrete page (namespace, class, function).
     *
     * @param {Object} page
     * @returns {{content:string,url:?string,urlType:?string}|null}
     */
    static buildPageBreadcrumbEntry(page) {
        if (!page) {
            return null
        }
        const rawContent = page.title || page.asciidoc?.title || page.src?.stem || page.src?.relative
        const content = CppReferenceExtension.stripHtml(rawContent)
        if (!content) {
            return null
        }
        const url = CppReferenceExtension.computePageUrl(page)
        return {content, url, urlType: url ? 'internal' : undefined}
    }

    /**
     * Convert an Antora virtual page into the canonical /component/module/path.html URL.
     *
     * @param {Object} page
     * @returns {string|null}
     */
    static computePageUrl(page) {
        if (!page) {
            return null
        }
        const src = page.src || {}
        const component = src.component || page.component?.name
        const moduleName = src.module || page.module || REFERENCE_MODULE_NAME
        if (!component || !moduleName) {
            return null
        }
        let relative = src.relative || ''
        const prefix = path.join('modules', moduleName, 'pages') + '/'
        ...
        const withoutExt = relative.replace(/\.adoc$/, '')
        return `/${component}/${moduleName}/${withoutExt}.html`
    }

    /**
     * Remove HTML tags from a breadcrumb label to keep the UI text clean.
     *
     * @param {string} value
     * @returns {string}
     */
    ...
    /**
     * Remove consecutive duplicate breadcrumbs so Reference → Reference collapses.
     *
     * @param {Array<{content:string,url:?string}>} entries
     * @returns {Array<{content:string,url:?string}>}
     */
    static deduplicateBreadcrumbs(entries) {
        const result = []
        ...
        for (const entry of entries) {
            ...
            const key = `${entry.content}::${entry.url || ''}`
            if (key === previousKey) {
                continue
            }
            result.push(entry)
